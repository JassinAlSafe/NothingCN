...
export default function CommunityPage() {
  ...
  useEffect(() => {
    async function fetchCommunityData() {
      try {
        const [topContributors, recentCommits, recentPRs, openIssues] = await Promise.all([
  ...
  const getIssueIcon = (labels: Array<{ name: string; color: string }>) => {
    ...
    if (labelNames.includes('bug')) return <Bug className="w-4 h-4 text-red-500" />;
    if (labelNames.includes('enhancement')) return <Zap className="w-4 h-4 text-blue-500" />;
    if (labelNames.includes('good first issue')) return <Heart className="w-4 h-4 text-green-500" />;
  ...
  const displayedActivities = showMoreActivity ? allActivities : allActivities.slice(0, 8);
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center space-y-6">
            ...
            <p className="max-w-3xl mx-auto text-lg text-muted-foreground">
              Meet our contributors, see recent activity, and find ways to get involved. 
      ...
      <section className="py-24 md:py-32">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            ...
            <div className="lg:col-span-1">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-bold text-2xl tracking-tight">Top Contributors</h2>
              ...
              <div className="space-y-4">
                {contributors.map((contributor, index) => (
                  <div key={contributor.id} className="flex items-center space-x-3 group hover:bg-muted/30 p-3 rounded-lg transition-colors">
            ...
            <div className="lg:col-span-1">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-bold text-2xl tracking-tight">Recent Activity</h2>
                <Badge className="bg-accent text-accent-foreground">
                  ...
                  <div key={date} className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {dayActivities.map((activity, index) => (
                        <Card key={index} className="border-0 shadow-none bg-muted/20 hover:bg-muted/40 transition-colors">
                          <CardContent className="p-3">
                            <div className="flex items-start space-x-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {activity.type === 'pullrequest' && activity.data.labels.length > 0 && (
                                  <div className="flex flex-wrap gap-1 mt-1">
                                    {activity.data.labels.slice(0, 2).map((label) => (
                                      <Badge 
                                        key={label.name} 
                                        variant="secondary" 
            ...
            <div className="lg:col-span-1">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-bold text-2xl tracking-tight">Help Wanted</h2>
                <Badge variant="secondary" className="text-xs">
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            <span className="text-xs text-muted-foreground">{getTimeAgo(issue.created_at)}</span>
                          </div>
          ...
          <div className="mt-16 pt-16 border-t border-border text-center">
            <h3 className="font-bold text-2xl mb-4">Ready to Join Our Community?</h3>
            <p className="text-muted-foreground mb-8 max-w-2xl mx-auto">
              Whether you're fixing bugs, adding features, or helping other contributors, 
              every contribution makes NothingCN better for everyone.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button asChild size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90">
                <Link href="https://github.com/JassinAlSafe/NothingCN/fork" target="_blank">
                  <Github className="mr-2 h-5 w-5" />
                  Fork Repository
                </Link>
              </Button>
              <Button variant="outline" size="lg" asChild>
                <Link href="https://github.com/JassinAlSafe/NothingCN/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22" target="_blank">
